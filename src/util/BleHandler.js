import bleUtils from "./BleUtils";
import base64 from "react-native-base64";
import stores from "../stores";
import { when } from "mobx";
import { Buffer } from "buffer";

export class BleHandler {
  constructor() {}
  enumerate() {
    // 列出设备列表, 可以通过 path 区分设备
    // console.log("enumerate ........+++");
    return Promise.resolve([{ path: "ble", debug: false }]);
  }
  acquire(path, debugLink) {
    return Promise.resolve().then(() => {
      console.log("acquire .........", path);
      stores.bleExchange.acquire();
    });
  }
  release(path, debugLink, closePort) {
    return Promise.resolve().then(() => {
      console.log("release .........", path);
      stores.bleExchange.release();
    });
  }
  write(path, debugLink, data) {
    let newArray = new Uint8Array(64);
    // add ? at the beginning of the data
    newArray[0] = 63;
    newArray.set(new Uint8Array(data), 1);
    // encode data to base64 format
    value = base64.encodeFromByteArray(newArray);
    bleUtils.writeWithoutResponse(value).then((characteristic) => {
      // console.log();
    });
    return Promise.resolve("");
  }
  async read(path, debugLink) {
    console.log("read .........");
    // return mock features data
    //TODO: need a read function to retrieve the ble response
    await when(() => stores.bleExchange.isReadDone);

    const data = stores.bleExchange.getData().toString('hex');
    console.log("数据接收完毕", data);
    return Promise.resolve({ data: data });
    // return Promise.resolve({ data: '' });
  }
}

export const Handler = new BleHandler();
